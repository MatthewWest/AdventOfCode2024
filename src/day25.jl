...
# ╔═╡ 1f03a899-a695-45bf-866d-0de39f170a5e
using Graphs, GraphPlot, Compose, Cairo, Fontconfig, Images

# ╔═╡ 7f975590-a35e-11ee-2dca-533ea107f0ac
INPUT = read(joinpath(@__DIR__, "../data/day25.txt"), String);

# ╔═╡ ac9c18fe-abf8-43be-9158-56194794c925
TEST_INPUT = """
jqt: rhn xhk nvd
rsh: frs pzl lsr
xhk: hfx
...
# ╔═╡ 4c0ab7b3-e16e-46ec-a70e-14ecd3f9eac3
function parse_input(s)
	vs = Set(split(strip(s), r"[^a-z]+"))
	es = Dict([v => String[] for v in vs])
	for line in split(strip(s), '\n')
		from, to_string = split(line, ':')
		tos = split(strip(to_string))
		for to in tos
			push!(es[from], to)
			push!(es[to], from)
		end
...
# ╔═╡ eed95518-57a3-4f2f-87eb-23b430a1adf6
function tograph(vs, es)
	G = SimpleGraph(length(vs))
	labelToInd = Dict([l => i for (i, l) in enumerate(vs)])
	for v in vs
		for e in es[v]
			add_edge!(G, labelToInd[v], labelToInd[e])
		end
	end
	G, labelToInd
end

# ╔═╡ 1adc2a79-695b-43e3-ad1f-6dbf3398fa36
function input_to_image(s = INPUT)
	vs, es = parse_input(s)
	G, labelToInd = tograph(vs, es)
	filename = "./day25graph.png"
	draw(
		PNG(filename, 100cm, 100cm),
		gplot(G, nodelabel=collect(vs)))
	load(filename)
end

# ╔═╡ a6f49efb-5ef5-4179-ba44-33ed69957651
img = input_to_image()

# ╔═╡ 87ac7a6c-2e2e-4dde-920f-0c0f91c08c52
md"""
By inspection of the image, we can see that there are two densely connected components, with 3 edges connecting them. Pluto shows a low-resolution version of the image, but if we look at it in a separate file browser, we can see that the edges that need to be cut to separate the clusters are:
...
I can write a function manually to remove those edges.
"""

# ╔═╡ e8f8efdf-1b99-4a9c-84a4-abe6f24e48d5
function Base.delete!(v::Vector{String}, el::String)
	inds = findall(==(el), v)
	deleteat!(v, inds)
end

# ╔═╡ 12094d85-62ad-47a6-8993-9af9140df96f
function input_to_answer(s = INPUT)
	vs, es = parse_input(s)
	delete!(es["ljl"], "xhg")
	delete!(es["xhg"], "ljl")
	delete!(es["lkm"], "ffj")
...
# ╔═╡ 0803e2be-e57b-419f-8e6a-3e42c51297e2
input_to_answer()
